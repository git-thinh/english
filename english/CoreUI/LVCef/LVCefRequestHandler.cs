namespace LVCef.ClientHandlers
{
    ...
    {
        static string[] _disable_Keyword = File.ReadAllLines("bin/disable.txt");

        bool f_valid_disableRequest(string url)
        {
            if (url.Contains(".js")) return false;

            if (url.Contains(".ttf") || url.Contains(".wotf") || url.Contains(".jpg") || url.Contains(".png") || url.Contains(".gif") || url.Contains(".jpeg") || url.Contains(".svg")) return true;

            for (var i = 0; i < _disable_Keyword.Length; i++)
                if (url.Contains(_disable_Keyword[i])) return false;
            return true;
        }
        ...
        protected override CefResourceHandler GetResourceHandler(CefBrowser browser, CefFrame frame, CefRequest request)
        {
            ...
            if (handler != null)
            {
                ...
                if (f_valid_disableRequest(url) == false)
                {
                    //Debug.WriteLine("----> CANCLE: " + url);
                    e.DelegateRequest = true;
                    return lvCefRequest;
                }
        ...
        private sealed class LVCefRequest : CefResourceHandler
        {
            ...
            protected override bool ProcessRequest(CefRequest request, CefCallback callback)
            {
                ...
                if (handler != null)
                {
                    ...
                    if (e.continueRequest == false)
                        Debug.WriteLine("----> CANCLE: " + request.Url);
                    else
                        Debug.WriteLine("----> FETCH: " + request.Url);
